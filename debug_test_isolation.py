...
class TestIsolationAnalyzer:
    ...
    def analyze_test_file(self, file_path: Path) -> List[TestIsolationIssue]:
        ...
        try:
            ...
            hardcoded_emails = self._find_hardcoded_emails(content, lines)
            
            # Find test methods
            test_methods = self._find_test_methods(content, lines)
            
            # Check for user creation without cleanup
            creates_users = self._check_user_creation(content)
            ...
            for method_name, method_lines in test_methods.items():
                ...
                method_emails = self._find_hardcoded_emails(method_content, 
                                                          lines[method_lines[0]:method_lines[1]])
                
                if method_emails:
                    issues.append(TestIsolationIssue(
                        ...
                        line_numbers=[i for i, line in enumerate(lines) 
                                    if any(email in line for email in method_emails)],
                        severity="HIGH"
                    ))
                
                # Check if creates users without cleanup
                if (self._check_user_creation(method_content) and 
                    not self._check_cleanup_patterns(method_content) and 
                    not has_cleanup):
                    issues.append(TestIsolationIssue(
                        test_file=str(file_path.relative_to(self.test_directory)),
                        test_method=method_name,
    ...
    def _find_test_methods(self, content: str, lines: List[str]) -> Dict[str, Tuple[int, int]]:
            ...
            if re.match(r'\s*(async\s+)?def\s+test_', line):
                ...
                if method_name:
                    ...
                    for j in range(i + 1, len(lines)):
                        if re.match(r'\s*(def|class)', lines[j]) and not lines[j].strip().startswith('#'):
                            end_line = j
                            break
    ...
    def generate_report(self) -> str:
            ...
            for issue_type, issues in by_type.items():
                ...
                for issue in issues:
                    ...
                    if issue.hardcoded_values:
                        report.append(f"    Values: {', '.join(issue.hardcoded_values)}")
                    if issue.line_numbers:
                        report.append(f"    Lines: {', '.join(map(str, issue.line_numbers[:5]))}")
                    report.append("")
        ...
        hardcoded_email_issues = [i for i in self.issues if i.issue_type == "HARDCODED_EMAIL"]
        ...
        if no_cleanup_issues:
            report.append("2. Add proper test cleanup using fixtures or teardown methods")
