...
def export_openapi_schema() -> None:
    ...
    try:
        ...
        available_schemas = openapi_schema.get("components", {}).get("schemas", {}).keys()
        
        missing_schemas = [schema for schema in required_schemas if schema not in available_schemas]
        if missing_schemas:
