...
async function runBackendTestsWithAutoSetup(testArgs = []) {
    try {
        ...
        const isIntegrationTest = testArgs.some(arg =>
        ...
        if (usePostgres && isIntegrationTest) {
            logger.info('Integration tests detected - ensuring PostgreSQL container is ready...');
            ...
            try {
                ...
                testEnv.REVIEWPOINT_DB_URL = 'postgresql+asyncpg://postgres:postgres@localhost:5432/reviewpoint_test';
