...
class PrerequisiteInstaller {
  ...
  async installViaChocolatey(packageName, name) {
    try {
      ...
      Logger.error(
        `Failed to install ${name} via Chocolatey: ${error.message}`,
      );
      this.failedTools.add(name);
      return false;
    }
  ...
  async checkAndInstallDocker() {
    ...
    Logger.info(
      "Please download and install from: https://docker.com/products/docker-desktop/",
    );
  ...
  async refreshEnvironment() {
    ...
    if (this.isWindows) {
      try {
        ...
        Logger.info(
          "Environment refresh completed (restart terminal if needed)",
        );
      }
    }
  }
  ...
  async installAllPrerequisites() {
    ...
    Logger.info(
      "This will install ALL required tools for ReViewPoint development",
    );
    ...
    if (this.installedTools.size > 0) {
      Logger.success(
        `Successfully installed: ${Array.from(this.installedTools).join(", ")}`,
      );
    }

    if (this.failedTools.size > 0) {
      Logger.error(
        `Failed to install: ${Array.from(this.failedTools).join(", ")}`,
      );
