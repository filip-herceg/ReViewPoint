...
class TestIsolationFixer:
    ...
    def fix_auth_test_file(self) -> List[FixAction]:
        ...
        if "from tests.test_data_generators import AuthTestDataGenerator" in content:
            print("   Auth test file already partially fixed - skipping to avoid conflicts")
            return []
        ...
        while i < len(lines):
            ...
            if method_match:
                ...
                has_generator = any("test_data = AuthTestDataGenerator" in line for line in next_few_lines)
                ...
                def replace_email(match):
                    ...
                    if email in self.hardcoded_emails:
                        ...
                        if "register" in line.lower() or "Register" in line:
                            return f'"{{test_data.get_registration_user().email}}"'
                        elif "login" in line.lower() or "Login" in line:
                            return f'"{{test_data.get_login_user().email}}"' 
                        elif "duplicate" in line.lower() or "dupe" in line.lower():
                            return f'"{{test_data.get_duplicate_test_users()[0].email}}"'
                        elif "reset" in line.lower() or "pwreset" in line.lower():
                            return f'"{{test_data.get_password_reset_user().email}}"'
                        else:
                            return f'"{{test_data.generate_unique_email()}}"'
        ...
        if not import_added:
            ...
            for i, line in enumerate(new_lines):
                if line.startswith("import") or line.startswith("from"):
                    ...
                    new_lines.insert(i, "from tests.test_data_generators import AuthTestDataGenerator")
                    break
    ...
    def _fix_hardcoded_emails_in_file(self, file_path: Path) -> List[FixAction]:
        ...
        for i, line in enumerate(lines):
            ...
            if not import_added and (line.startswith("from tests.") or line.startswith("from src.")):
                new_lines.append("from tests.test_data_generators import get_unique_email, get_test_user")
                import_added = True
            ...
            for pattern in self.email_patterns:
                ...
                if new_content != modified_line:
                    fixes.append(FixAction(
                        ...
                        new_content=new_content,
                        fix_type="REPLACE_HARDCODED_EMAIL",
                        description=f"Replaced hardcoded email with unique generator"
                    ))
                    modified_line = new_content
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fix test isolation issues automatically")
    ...
    try:
        ...
        # Apply fixes
        all_fixes = fixer.fix_all_test_files()
