...
def skip_if_fast_tests(reason: Optional[str] = None):
    ...
    if os.environ.get("FAST_TESTS") == "1":
        default_reason = "Test not compatible with fast test mode (SQLite in-memory)"
...
def skip_if_not_fast_tests(reason: Optional[str] = None):
    ...
    """
    if os.environ.get("FAST_TESTS") != "1":
        default_reason = "Test only runs in fast test mode"
        pytest.skip(reason or default_reason)
...
def skip_if_missing_feature(feature_name: str, reason: Optional[str] = None):
    ...
    try:
        ...
        default_reason = f"Feature '{feature_name}' not available"
        pytest.skip(reason or default_reason)


# Pytest markers for use with @pytest.mark.skip_if_fast_tests
skip_if_fast_tests_marker = pytest.mark.skip_if_fast_tests
skip_if_not_fast_tests_marker = pytest.mark.skip_if_not_fast_tests
requires_real_db_marker = pytest.mark.requires_real_db
requires_timing_precision_marker = pytest.mark.requires_timing_precision
