...
class TestGetCurrentUser(AuthUnitTestTemplate):
    @pytest.mark.asyncio
    async def test_get_current_user_returns_dev_user_when_auth_disabled(self, async_session: AsyncSession) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_invalid_token(self, async_session: AsyncSession) -> None:
        from src.api import deps
        from src.core.config import get_settings
        settings = get_settings()
        self.patch_setting(settings, "auth_enabled", True)
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_user_not_found(self, async_session: AsyncSession) -> None:
        from src.api import deps
        from src.core.config import get_settings
        settings = get_settings()
        self.patch_setting(settings, "auth_enabled", True)
