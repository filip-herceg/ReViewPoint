...
@pytest.mark.asyncio
async def test_get_current_user_user_inactive(
    monkeypatch: pytest.MonkeyPatch, async_session: AsyncSession
) -> None:
    monkeypatch.setattr(settings, "auth_enabled", True)
    ...
    monkeypatch.setattr(deps, "get_user_by_id", AsyncMock(return_value=FakeUser()))
    with pytest.raises(HTTPException) as exc:
        await deps.get_current_user(token="token", session=async_session)
    assert exc.value.status_code == 401
    assert "User not found" in str(exc.value.detail)
...
@pytest.mark.asyncio
async def test_get_current_user_user_deleted(
    monkeypatch: pytest.MonkeyPatch, async_session: AsyncSession
) -> None:
    monkeypatch.setattr(settings, "auth_enabled", True)
