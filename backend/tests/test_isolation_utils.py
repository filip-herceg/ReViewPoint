    ...
    async def cleanup_by_email_pattern(self, email_pattern: str) -> None:
        ...
        try:
            ...
            if deleted_count and deleted_count > 0:
                logger.debug(
                    f"Cleaned up {deleted_count} users matching pattern: {email_pattern}"
                )
        except Exception as e:
...
) -> AsyncGenerator[TestIsolationManager, None]:
    """
        ...
        for table, initial_count in self.initial_state.items():
            try:
                ...
                if difference != 0:
                    ...
                    logger.warning(
                        f"State change detected in {table}: {difference} records"
                    )
...
@asynccontextmanager
async def verified_test_isolation(
    session: AsyncSession,
) -> AsyncGenerator[tuple[TestIsolationManager, DatabaseStateVerifier], None]:
    """
        ...
        for pattern in patterns:
            try:
                ...
                if table == "used_password_reset_tokens":
                    ...
                    result = await session.execute(
                        text(
                            f"DELETE FROM {table} WHERE id IN (SELECT id FROM {table} LIMIT 0)"
                        )
                    )
                cleaned = getattr(result, "rowcount", 0) or 0
                total_cleaned += cleaned
                logger.debug(
                    f"Cleaned {cleaned} records from {table} matching pattern: {pattern}"
                )
            except Exception as e:
