...
class TestUserService(AuthUnitTestTemplate):
    ...
    async def test_success(self) -> None:
        ...
        with patch("src.repositories.user.update_last_login", new_callable=AsyncMock):
            tokens: tuple[str, str] = await user_service.authenticate_user(
                session, dummy.email, "pw"
            )
        access_token: str
        refresh_token: str
    ...
    def test_inactive(self) -> None:
        ...
        user: User = cast(
            "User", self.make_real_user(is_active=False, is_deleted=False)
        )
    ...
    async def test_delete_soft(self) -> None:
        ...
        result: bool = await user_service.delete_user_account(
            session, 1, anonymize=False
        )
        assert result is True
    ...
    async def test_errors(
        self, error_case: str, expected_exception: str, expected_msg: str
    ) -> None:
        """
                                ...
                                with ctx3:
                                    if len(patchers) > 3:
                                        ...
                                        with ctx4:
                                            ...
                                            if error_case == "jwt_decode_error":
                                                assert "Unexpected error: fail" in str(
                                                    exc.value
                                                )
                                            else:
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            with patch(
                                "src.services.user.get_user_by_id",
                                new=AsyncMock(return_value=real_user),
                            ):
                                result: str = (
                                    await user_service.async_refresh_access_token(
                                        session, token, jwt_secret, jwt_algorithm
...
class DummyResult:
    ...
    def __init__(self, user: DummyUser | None) -> None:
        self._user: DummyUser | None = user

    def scalar_one_or_none(self) -> DummyUser | None:
        return self._user
