...
def collect_py_files(root: pathlib.Path, ignored: Set[str]) -> Set[pathlib.Path]:
    def is_in_excluded_dir(p: pathlib.Path) -> bool:
        ...
        return any(
            ...
            for suffix in ["/alembic_migrations/versions", "/alembic_migrations/versions/"]
        )
    return {
        p for p in root.rglob("*.py")
        if p.is_file() and p.name not in ignored and not any(
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    def color(text, color):
        return f"\x1b[" + {"red": "31", "green": "32", "yellow": "33", "blue": "34", "magenta": "35", "cyan": "36"}.get(color, "0") + "m" + text + "\x1b[0m"

    def is_excluded(src, test):
        return (src.name, test.name) in EXCLUDED_MAPPINGS

    missing_tests = [
        (src, test) for src, test in result.missing_tests
        if not is_excluded(src, test)
    ]
    missing_sources = [
        (test, src) for test, src in result.missing_sources
        if not is_excluded(src, test)
    ]
    ...
    missing_sources = [
        (test, src) for test, src in missing_sources
        if test.name not in excluded_test_names
    ]

    # Sort missing_tests and missing_sources alphabetically by relative path
    missing_tests = sorted(missing_tests, key=lambda pair: str(pair[0].relative_to(SRC_ROOT)))
    missing_sources = sorted(missing_sources, key=lambda pair: str(pair[0].relative_to(TESTS_ROOT)))
    ...
    if missing_tests or missing_sources or result.multiply_mapped:
        ...
        if missing_tests:
            term.write_line("")
            term.write_line(color(f"Missing test files for ({len(missing_tests)}) source file(s):", "yellow"))
            for src, test in missing_tests:
                term.write_line(f"  {paint_path(src, SRC_ROOT)}")
        if missing_sources:
            term.write_line("")
            term.write_line(color(f"Test files with no corresponding source file ({len(missing_sources)}):", "yellow"))
            for test, src in missing_sources:
                term.write_line(f"  {paint_path(test, TESTS_ROOT)}")
        if result.multiply_mapped:
            term.write_line("")
            term.write_line(color(f"Multiply-mapped test files ({len(result.multiply_mapped)}):", "yellow"))
            for t in sorted(result.multiply_mapped, key=lambda p: str(p.relative_to(TESTS_ROOT))):
                term.write_line(f"  {paint_path(t, TESTS_ROOT)}")
        term.write_line("")
        term.write_line(color("Please fix the mapping errors above.", "red"))
    else:
        if term.verbosity > 0:
            term.section(color("[pytest-mapping-checker] 1:1 source-test mapping check PASSED", "green"), sep="-")
            term.write_line(color(f"  Total source files checked: {result.src_files}", "green"))
            term.write_line(color(f"  Total test files checked:   {result.test_files}", "green"))
            term.write_line(color("  All source files have matching test files and vice versa.", "green"))
        else:
            term.write_line(color("[pytest-mapping-checker] 1:1 source-test mapping check PASSED: every source file has a matching test file and vice versa.", "green"))
