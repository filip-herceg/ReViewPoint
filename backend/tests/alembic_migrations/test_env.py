...
def test_run_migrations_offline_success(monkeypatch):
    ...
    fake_config.get_main_option = lambda key: "sqlite:///:memory:" if key == "sqlalchemy.url" else None
...
def test_run_migrations_online_success(monkeypatch):
    ...
    def fake_engine_from_config(*a, **k): return FakeEngine()
...
def test_run_migrations_online_no_url(monkeypatch):
    ...
    monkeypatch.setattr("alembic.context", fake_context)
    monkeypatch.setattr("logging.config.fileConfig", lambda *a, **k: None)
    def fake_engine_from_config(*a, **k): return mock.Mock(connect=lambda: mock.Mock(__enter__=lambda s: s, __exit__=lambda s, a, b, c: False))
    with pytest.raises(ValueError):
        env.run_migrations_online(fake_engine_from_config)
...
def test_run_migrations_online_happy_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert context_mod.configure.called, "context.configure should be called in online mode"
    assert context_mod.begin_transaction.called, "context.begin_transaction should be called in online mode"
    assert context_mod.run_migrations.called, "context.run_migrations should be called in online mode"
...
def test_run_migrations_offline_configures_logging(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import src.alembic_migrations.env as env
    env.run_migrations_offline()
    assert file_config_mock.called, "logging.config.fileConfig should be called in offline mode"
def test_run_migrations_online_configures_logging(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import src.alembic_migrations.env as env
    env.run_migrations_online(lambda *a, **kw: mock_engine)
    assert file_config_mock.called, "logging.config.fileConfig should be called in online mode"
def test_run_migrations_offline_fileConfig_not_called_when_no_config_file(monkeypatch: pytest.MonkeyPatch) -> None:
    context_mod = types.SimpleNamespace()
    def get_main_option(key: str) -> str:
        return f"sqlite:///{TEST_DB_PATH}"
    config_mock = mock.Mock()
    ...
    context_mod.begin_transaction = mock.MagicMock()
    context_mod.run_migrations = mock.MagicMock()
    monkeypatch.setitem(sys.modules, "alembic", types.SimpleNamespace(context=context_mod))
    monkeypatch.setitem(sys.modules, "alembic.context", context_mod)
...
def test_run_migrations_offline_fileConfig_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="fileConfig failed"):
        env.run_migrations_offline()
def test_run_migrations_offline_begin_transaction_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    context_mod = types.SimpleNamespace()
    def get_main_option(key: str) -> str:
        return f"sqlite:///{TEST_DB_PATH}"
    config_mock = mock.Mock()
    ...
    context_mod.config = config_mock
    context_mod.configure = mock.MagicMock()
    begin_transaction_mock = mock.MagicMock(side_effect=RuntimeError("begin_transaction failed"))
...
def test_run_migrations_offline_run_migrations_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    context_mod.configure = mock.MagicMock()
    context_mod.begin_transaction = mock.MagicMock()
    run_migrations_mock = mock.MagicMock(side_effect=RuntimeError("run_migrations failed"))
    ...
    with pytest.raises(RuntimeError, match="run_migrations failed"):
        env.run_migrations_offline()
def test_run_migrations_online_engine_from_config_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    context_mod = types.SimpleNamespace()
    def get_main_option(key: str) -> str:
        return f"sqlite:///{TEST_DB_PATH}"
    config_mock = mock.Mock()
