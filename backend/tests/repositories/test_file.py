...
async def test_create_file_success(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="test@example.com", hashed_password="hashed", is_active=True)
...
async def test_create_file_missing_filename(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="test2@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_get_file_by_filename(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="test3@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_delete_file(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="test4@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_duplicate_filename_same_user(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="dupuser@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_long_filename_and_content_type(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="test5@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_non_ascii_and_special_chars(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="test6@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_bulk_create_and_delete(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="test7@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_list_files_pagination_and_search(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="pagsearch@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
...
async def test_list_files_sorting(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="sortuser@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
    user_id: Final[int] = user.id
    now: Final[datetime] = datetime.now(UTC)
    ...
    # Normalize datetimes to naive for SQLite compatibility
    created_ats_naive = [dt.astimezone(UTC).replace(tzinfo=None) if dt.tzinfo is not None else dt for dt in created_ats]
...
async def test_transactional_rollback_create_file(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="rollbackuser@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
    user_id = user.id
...
async def test_rollback_after_integrity_error(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="rollbackdupuser@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
    user_id = user.id
...
async def test_simulated_concurrent_creation(async_session: AsyncSession) -> None:
    ...
    user: User = User(email="concurrentuser@example.com", hashed_password="hashed", is_active=True)
    async_session.add(user)
    await async_session.flush()
    user_id: Final[int] = user.id
    await create_file(
