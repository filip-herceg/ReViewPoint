...
class UploadService:
    ...
    async def get_file(self, session: AsyncSession, filename: str) -> Optional[File]:
        """Get file metadata by filename."""
        return await get_file_by_filename(session, filename)
    
    async def delete_file(self, session: AsyncSession, filename: str, user_id: uuid.UUID) -> bool:
