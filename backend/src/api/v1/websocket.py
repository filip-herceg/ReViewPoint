...
class WebSocketConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user: User) -> str:
        ...
        logger.info(
            f"[WS] New connection established",
            extra={
                "connection_id": connection_id,
                "user_id": user_id,
                "user_email": user.email,
    ...
    async def send_to_user(self, user_id: str, message: Dict[str, Any]) -> int:
        ...
        for websocket in connections:
            try:
                ...
                logger.error(
                    f"[WS] Failed to send message to connection",
                    extra={
                        "user_id": user_id,
                        "error": str(e),
    ...
    async def broadcast_to_all(self, message: Dict[str, Any]) -> int:
        ...
        logger.info(
            f"[WS] Message broadcasted to all users",
            extra={
                "message_type": message.get("type", "unknown"),
        ...
        while True:
            try:
                ...
                if message_type == "ping":
                    ...
                    logger.info(
                        f"[WS] User subscribed to events",
                        extra={
                            "user_id": str(user.id),
                    ...
                    logger.warning(
                        f"[WS] Unknown message type received",
                        extra={
                            "user_id": str(user.id),
...
async def broadcast_upload_progress(user_id: str, upload_id: str, progress: int) -> None:
    """
...
async def broadcast_upload_completed(user_id: str, upload_id: str, result: Dict[str, Any]) -> None:
    """
