...
from src.api.deps import get_current_user, optional_get_current_user
...
async def register(
    ...
    try:
        ...
        token = user_service.create_access_token({"sub": str(user.id), "email": user.email})
        logger.info("registration_success", extra={"user_id": user.id, "email": user.email})
        ...
        logger.error("registration_unexpected_error", extra={"email": data.email, "error": str(e)})
...
async def login(
    ...
    try:
        ...
        logger.error("login_unexpected_error", extra={"email": data.email, "error": str(e)})
        raise HTTPException(status_code=500, detail="An unexpected error occurred. Please try again later.") from e
    ...
    if auth_header and auth_header.lower().startswith("bearer "):
        ...
        try:
            ...
            if exp:
                ...
                logger.info("logout_token_blacklisted", extra={"user_id": current_user.id})
        except Exception as e:
            logger.warning("logout_token_blacklist_failed", extra={"user_id": current_user.id, "error": str(e)})
...
async def refresh_token(
    ...
    try:
        ...
        if await is_token_blacklisted(session, jti):
            logger.warning("refresh_token_blacklisted", extra={"user_id": current_user.id})
            raise HTTPException(status_code=401, detail="Invalid or expired refresh token.")
    except Exception as e:
        logger.warning("refresh_token_decode_failed", extra={"user_id": current_user.id, "error": str(e)})
        raise HTTPException(status_code=401, detail="Invalid or expired refresh token.")
    try:
        ...
        logger.error("refresh_unexpected_error", extra={"user_id": current_user.id, "error": str(e)})
        raise HTTPException(status_code=500, detail="An unexpected error occurred. Please try again later.") from e
...
async def request_password_reset(
    ...
    try:
        ...
        logger.warning("pwreset_request_failed", extra={"email": data.email, "error": str(e)})
        return MessageResponse(message="Password reset link sent.")
...
async def reset_password(
    ...
    allowed = await user_action_limiter.is_allowed(limiter_key)
    if not allowed:
        logger.warning("pwreset_confirm_rate_limited", extra={"token_prefix": data.token[:8]})
    ...
    if pw_error:
        logger.warning("pwreset_confirm_invalid_password", extra={"token_prefix": data.token[:8]})
        raise HTTPException(status_code=400, detail=pw_error)
    ...
    try:
        ...
        logger.warning("pwreset_confirm_validation_error", extra={"token_prefix": data.token[:8]})
        raise HTTPException(status_code=400, detail=str(e)) from e
    except Exception as e:
        logger.warning("pwreset_confirm_failed", extra={"token_prefix": data.token[:8], "error": str(e)})
        raise HTTPException(status_code=400, detail="An error occurred while resetting the password. Please try again later.") from e
