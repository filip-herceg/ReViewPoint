...
```typescript
async function updateUser(userId: number, userData: UserCreateRequest): Promise<User>
```
...
class UserManager {
  ...
  async searchUsers(searchTerm: string, page: number = 0, pageSize: number = 20) {
    try {
      const searchParams = {
  ...
  async deleteMultipleUsers(userIds: number[]): Promise<{success: number[], failed: number[]}> {
...
async function analyzeUserRegistrations() {
  try {
    ...
    const analytics = {
      ...
      growthRate: ((recentUsers.total - olderUsers.total) / olderUsers.total) * 100
    };
...
logger.warn("User creation failed", { error: response.error, email: userData.email });
logger.info("User created successfully", { id: response.data?.id, email: response.data?.email });

logger.info("Fetching users list", { params });
logger.info("Users fetched successfully", { total: response.data?.total, count: response.data?.users.length });

logger.info("Updating user", { userId, email: userData.email });
logger.warn("User update failed", { error: response.error, userId });
logger.info("User updated successfully", { id: response.data?.id, email: response.data?.email });
```
...
describe('usersCoreApi', () => {
  ...
  describe('createUser', () => {
    ...
    it('should handle creation errors', async () => {
      mockRequest.mockResolvedValue({ data: null, error: 'Email already exists' });

      await expect(usersCoreApi.createUser({
        email: 'test@example.com',
        name: 'Test User',
        password: 'password123'
