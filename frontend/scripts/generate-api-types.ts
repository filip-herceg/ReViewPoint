...
async function validateSchema(): Promise<any> {
    ...
    try {
        ...
        logger.debug('Schema info:', {
            ...
            schemasCount: Object.keys((schema as any).components?.schemas || {}).length,
        });
        ...
        if ((error as any).code === 'ENOENT') {
            ...
            logger.error('💡 Run "pnpm run generate:openapi-schema" first to export the schema from backend');
        } else {
...
async function writeFiles(typesOutput: string): Promise<void> {
    ...
    try {
        ...
        logger.info('  📄 Schema types:', path.relative(FRONTEND_ROOT, SCHEMA_OUTPUT));
        logger.info('  📄 API client:', path.relative(FRONTEND_ROOT, CLIENT_OUTPUT));
        logger.info('  📄 Validators:', path.relative(FRONTEND_ROOT, VALIDATORS_OUTPUT));
...
async function generateApiTypes(): Promise<void> {
    ...
    try {
        ...
        logger.info(`🎉 API type generation completed successfully in ${duration}ms`);
...
async function main(): Promise<void> {
    try {
        ...
        if (error instanceof Error && error.message.includes('ENOENT')) {
            logger.error('💡 Make sure to run "pnpm run generate:openapi-schema" first');
        }
        process.exit(1);
    }
}
