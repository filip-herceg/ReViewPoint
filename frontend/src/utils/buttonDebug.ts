 ...
 */
export function debugButtonChildren(children: React.ReactNode, componentName = 'Button') {
  ...
  if (childCount > 1) {
    console.warn('❌ Multiple children detected. React.Children.only will fail.');
...
export function validateAsChildUsage(children: React.ReactNode): {
  ...
  if (childCount > 1) {
    return {
      ...
      suggestion: 'Wrap multiple elements in a single container element (e.g., <div> or <span>)'
    };
  }
...
export function findButtonAsChildUsages(component: React.ComponentType) {
  // This would need to be implemented with React DevTools or testing utilities
  console.warn('findButtonAsChildUsages: Not implemented - use React DevTools to inspect component tree');
}
