...
const categoryColors = {
    ...
    analysis: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
    formatting: 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400',
    collaboration: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400',
    ...
    automation: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400'
};
        ...
        <div className="container mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                    <Card>
                        <CardContent className="p-4">
                            <div className="text-2xl font-bold">{marketplaceStats.totalModules}</div>
                    ...
                    <Card>
                        <CardContent className="p-4">
                            <div className="text-2xl font-bold">{marketplaceStats.featuredModules.length}</div>
                            <div className="text-sm text-muted-foreground">Featured Modules</div>
                        </CardContent>
                    </Card>
                </div>
            </div>
                                        ...
                                        {Object.keys(categoryIcons).map((category) => (
                                            <div key={category} className="flex items-center space-x-2">
                                                <Checkbox
                                                    ...
                                                    checked={searchParams.filters?.category?.includes(category) || false}
                                    ...
                                    <div className="space-y-2">
                                        {['free', 'freemium', 'paid', 'subscription'].map((type) => (
                                            <div key={type} className="flex items-center space-x-2">
                                                <Checkbox
                                                    id={`license-${type}`}
                                                    checked={searchParams.filters?.licenseType?.includes(type) || false}
                                                    onCheckedChange={(checked: boolean) => {
                                                        const currentTypes = searchParams.filters?.licenseType || [];
                                                ...
                                                />
                                                <label htmlFor={`license-${type}`} className="text-sm capitalize cursor-pointer">
                                                    {type}
                                                </label>
                                            </div>
                                        ))}
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex items-center space-x-2">
                                            <Checkbox
                                                ...
                                                onCheckedChange={(checked: boolean) => handleFilterChange({ verified: checked || undefined })}
                                            />
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <Checkbox
                                                ...
                                                onCheckedChange={(checked: boolean) => handleFilterChange({ featured: checked || undefined })}
                                            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                {modules.map((module) => {
                    ...
                    return (
                        <Card key={module.id} className="h-full flex flex-col hover:shadow-lg transition-shadow">
                            <CardHeader className="pb-4">
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-2">
                                        <IconComponent className="h-6 w-6 text-primary" />
                                        {module.featured && <Crown className="h-4 w-4 text-yellow-500" />}
                                        {module.verified && <Verified className="h-4 w-4 text-blue-500" />}
                                    </div>
                ...
                <div className="flex justify-center gap-2">
                    ...
                    <div className="flex gap-1">
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                            <Button
                                key={page}
                                variant={(searchParams.page || 1) === page ? "default" : "outline"}
                                size="sm"
