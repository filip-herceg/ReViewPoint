...
import { type SortField, type SortOrder } from '@/components/file-management/FileToolbar';
...
// Error types
export type FileManagementErrorType = 'network_error' | 'server_error' | 'not_found' | 'unauthorized' | 'forbidden' | 'validation_error';
...
interface FileManagementState {
    ...
    bulkDelete: (filenames: string[]) => Promise<{ deleted: string[], failed: string[] }>;
...
export const useFileManagementStore = create<FileManagementState>((set, get) => ({
    ...
    fetchFiles: async (options = {}) => {
        ...
        try {
            ...
            const result = await uploadApiClient.listFiles({
                ...
                q: search || undefined,
                sort: sortField === 'size' || sortField === 'content_type' ? 'filename' : sortField,
                order: sortOrder
            });
            ...
            const mappedFiles = result.files.map(file => ({
                ...
                selected: file.filename ? get().selectedFiles.includes(file.filename) : false
            }));
    ...
    deselectFiles: (filenames: string[]) => {
        set(state => ({
            selectedFiles: state.selectedFiles.filter(f => !filenames.includes(f)),
            files: state.files.map(file =>
                filenames.includes(file.filename)
                    ? { ...file, selected: false }
                    : file
    ...
    bulkDelete: async (filenames: string[]) => {
        ...
        try {
            ...
            set(state => ({
                files: state.files.filter(f => !result.deleted.includes(f.filename)),
                totalFiles: Math.max(0, state.totalFiles - result.deleted.length),
                selectedFiles: state.selectedFiles.filter(f => !result.deleted.includes(f))
            }));
...
export function createTestFileItem(overrides: Partial<FileItem> = {}): FileItem {
    return {
...
export function createTestFileManagementState(overrides: Partial<FileManagementState> = {}) {
