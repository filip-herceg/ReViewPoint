...
import { handleApiError, type HandledError } from '@/lib/api/errorHandling'
import type { Upload, UploadCreateRequest } from '@/lib/api/types'
import { validateUploadStatus } from '@/lib/api/types/upload';
import { UploadStatus } from '@/lib/api/types';
...
export const useUploadStore = create<UploadState>((set, get) => ({
    ...
    fetchUploads: async () => {
        ...
        try {
            ...
            const uploads: Upload[] = Array.isArray(res.files) ? res.files.map(file => ({
                id: file.filename || `file-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                name: file.filename || 'Unknown File',
                status: validateUploadStatus(file.status || '') ? (file.status as UploadStatus) : 'completed',
                progress: typeof file.progress === 'number' ? file.progress : 100,
                createdAt: file.createdAt || new Date().toISOString(),
                response: file.url ? { filename: file.filename, url: file.url } : undefined
            })) : [];
    ...
    createUpload: async (upload: UploadCreateRequest) => {
        ...
        try {
            ...
            const newUpload: Upload = {
                id: upload.id || `upload-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                name: upload.name,
            ...
            set((state) => ({ uploads: [...state.uploads, newUpload], loading: false }));
        } catch (err) {
            const handledError = handleApiError(err);
            set({ error: handledError, loading: false });
        }
