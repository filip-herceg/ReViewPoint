        ...
        (set, get) => ({
            ...
            setTheme: (theme: ThemeMode) => {
                if (theme !== 'light' && theme !== 'dark') {
                    const error = new Error(`Invalid theme provided: ${theme}. Expected 'light' or 'dark'.`);
            ...
            toggleSidebar: () => {
                set((state) => {
                    ...
                    logger.debug('Sidebar toggled', { from: state.sidebarOpen, to: newOpen });
            ...
            toggleSidebarCollapsed: () => {
                set((state) => {
                    ...
                    logger.debug('Sidebar collapsed toggled', { from: state.sidebarCollapsed, to: newCollapsed });
            ...
            addNotification: (notification: Omit<Notification, 'id'>) => {
                if (!notification.title?.trim()) {
                    ...
                    logger.error('Notification title is required', { notification, error });
                    throw error;
                }
            ...
            removeNotification: (id: string) => {
                if (!id?.trim()) {
                    const error = new Error('Invalid notification ID: ID cannot be empty.');
                ...
                if (!notificationExists) {
                    ...
                    logger.error('Notification not found', { id, error, existingIds: state.notifications.map(n => n.id) });
                    throw error;
                }
