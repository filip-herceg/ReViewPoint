...
class TokenService {
    ...
    decodeJWTPayload(token: string): JWTPayload | null {
        try {
            ...
            if (parts.length !== 3) {
                logger.warn('[TokenService] Invalid JWT format', { tokenLength: token.length });
                return null;
            }
    ...
    needsRefresh(): boolean {
        ...
        logger.debug('[TokenService] Token refresh check', { needsRefresh: expired });
        return expired;
    }
    ...
    async refreshAccessToken(): Promise<string> {
        ...
        if (this.refreshState.isRefreshing && this.refreshState.refreshPromise) {
            logger.debug('[TokenService] Token refresh in progress - queuing request');
        ...
        if (!refreshToken) {
            ...
            logger.error('[TokenService] Refresh failed - no refresh token', { error: error.message });
