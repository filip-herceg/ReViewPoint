 ...
 */
export function getFileCategory(contentType: string): keyof typeof FILE_TYPE_CATEGORIES | 'OTHER' {
 ...
 */
export function isValidFileType(file: globalThis.File, allowedTypes: string[]): boolean {
 ...
 */
export function isValidFileSize(file: globalThis.File, maxSize: number): boolean {
 ...
 */
export function calculateUploadProgress(uploadedOrUpload: number | Upload, total?: number): number {
 ...
 */
export interface FileValidationWarning extends Omit<FileValidationError, 'severity'> {
    severity: 'warning';
}
 ...
 */
export type FileTypeCategory = 'document' | 'image' | 'archive' | 'data' | 'other';
...
export interface FileUpdateEvent {
    /** Event type */
    type: 'file_created' | 'file_updated' | 'file_deleted' | 'file_shared' | 'upload_progress';
    /** File ID */
    fileId: number;
    /** Updated file data */
    data: Partial<File>;
