...
import { generatedApiClient, getApiErrorMessage } from '../generated/client';
...
export const uploadApiClient = {
    ...
    async listFiles(params?: {
        ...
        try {
            ...
            if (!data) {
                ...
                throw new Error('Failed to fetch file list: No data returned from server');
            }

            logger.info('✅ File list fetched successfully:', { count: data.files?.length || 0 });
            return data;
    ...
    async getFileInfo(filename: string): Promise<UploadFileResponse> {
        try {
            ...
            if (!data) {
                ...
                throw new Error('Failed to fetch file info: No data returned from server');
            }

            logger.info('✅ File info fetched successfully:', { filename, url: data.url });
            return data;
    ...
    async downloadFile(filename: string): Promise<Blob> {
        try {
            ...
            const baseUrl = (import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:8000';
            const response = await fetch(`${baseUrl}/api/v1/uploads/${filename}/download`, {
                headers: {
            ...
            logger.info('✅ File downloaded successfully:', { filename, size: blob.size });
            return blob;
    ...
    async deleteFile(filename: string): Promise<void> {
        try {
            ...
            const { data, error } = await generatedApiClient.DELETE('/api/v1/uploads/{filename}', {
                params: {
                    path: { filename },
                },
            });
     ...
     */
    async bulkDeleteFiles(filenames: string[]): Promise<{ deleted: string[]; failed: string[] }> {
        try {
            logger.info('🗑️ Bulk deleting files:', { count: filenames.length, filenames });
            ...
            const result = await response.json() as { deleted: string[]; failed: string[] };
