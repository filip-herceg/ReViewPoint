...
export const authApi = {
    // Register a new user
    register: async (userData: AuthRegisterRequest): Promise<AuthRegisterResponse> => {
        ...
        const response = await request<AuthRegisterResponse>('/api/v1/auth/register', {
            method: 'POST',
            data: registrationData,
        });

        if (response.error) {
            logger.warn('User registration failed', { error: response.error, email: userData.email });
    ...
    login: async (loginData: AuthLoginRequest): Promise<AuthLoginResponse> => {
        ...
        if (response.error) {
            logger.warn('User login failed', { error: response.error, email: loginData.email });
            throw new Error(response.error);
        }
    ...
    refreshToken: async (refreshToken: string): Promise<AuthTokenRefreshResponse> => {
        logger.info('Refreshing access token');
        const response = await request<AuthTokenRefreshResponse>('/api/v1/auth/refresh-token', {
            method: 'POST',
    ...
    forgotPassword: async (resetRequest: AuthPasswordResetRequest): Promise<AuthPasswordResetResponse> => {
        logger.info('Requesting password reset', { email: resetRequest.email });
        const response = await request<AuthPasswordResetResponse>('/api/v1/auth/request-password-reset', {
            method: 'POST',
            data: resetRequest,
        });

        if (response.error) {
            logger.warn('Password reset request failed', { error: response.error, email: resetRequest.email });
            throw new Error(response.error);
        }

        logger.info('Password reset requested successfully', { email: resetRequest.email });
        return response.data!;
    },

    // Request password reset (original method name)
    requestPasswordReset: async (email: string): Promise<AuthPasswordResetResponse> => {
        return authApi.forgotPassword({ email });
    },

    // Reset password with token
    resetPassword: async (resetRequest: AuthPasswordResetConfirmRequest): Promise<AuthPasswordResetConfirmResponse> => {
        logger.info('Resetting password with token');
        const response = await request<AuthPasswordResetConfirmResponse>('/api/v1/auth/reset-password', {
            method: 'POST',
            data: resetRequest,
        });
    ...
    getCurrentUser: async (): Promise<User> => {
        ...
        if (response.error) {
            logger.warn('Failed to get current user profile', { error: response.error });
            throw new Error(response.error);
        }
