...
class WebSocketService {
	...
	private eventListeners: Map<WebSocketEventType, Set<EventListener>> =
		new Map();
		...
		if (this.rateLimitState.messageCount >= this.config.rateLimitMaxMessages) {
			if (!this.rateLimitState.isLimited) {
				...
				this.rateLimitState.resetTime = new Date(
					this.rateLimitState.windowStart.getTime() +
	...
	private clearPendingPings(): void {
		for (const [_id, { timeout }] of this.pendingPings) {
	...
	private handlePongResponse(message: WebSocketMessage): void {
		...
		if (!pingId || !this.pendingPings.has(pingId)) {
			logger.debug("[WebSocket] Received pong without matching ping", {
				pingId,
			});
			return;
		}
	   ...
	   public send(type: WebSocketEventType, data: Record<string, unknown> = {}): void {
		...
		if (this.state === "connected" && this.ws?.readyState === WebSocket.OPEN) {
			...
			if (this.config.enableMessageQueue) {
				...
				logger.warn(
					"[WebSocket] Message dropped (not connected, queue disabled)",
					{ type },
				);
			}
		}
	...
	public getStats(): ConnectionStats {
		return {
			...
			activeListeners: Array.from(this.eventListeners.entries()).map(
				([event, listeners]) => ({
					event,
