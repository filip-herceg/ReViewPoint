 ...
 */
export type ValidationRule = (file: File) => Promise<FileValidationError[]> | FileValidationError[];
...
const FILE_SIGNATURES: Record<string, { pattern: number[]; offset: number }[]> = {
    ...
    'image/png': [{ pattern: [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A], offset: 0 }], // PNG
    'image/gif': [
...
const DEFAULT_VALIDATION_CONFIG: ValidationConfig = {
    basic: {
        ...
        allowedExtensions: Object.keys(SUPPORTED_FILE_TYPES).map(k => k.toLowerCase()),
    },
    security: {
...
export class FileValidator {
        ...
        try {
            ...
            for (const rule of this.config.customRules) {
                try {
                    ...
                    logger.error('Custom validation rule failed', { error, fileName: file.name });
                    errors.push({
    ...
    private validateBasic(file: File): FileValidationError[] {
        ...
        if (extension && !this.config.basic.allowedExtensions.includes(extension.toLowerCase())) {
            errors.push({
    ...
    private async validateSecurity(file: File): Promise<FileValidationError[]> {
        ...
        if (this.config.security.validateFileHeaders) {
            try {
                ...
                logger.error('File header validation failed', { error, fileName: file.name });
                errors.push({
        ...
        if (this.config.security.checkEmbeddedScripts && this.isTextBasedFile(file)) {
            try {
                const scriptErrors = await this.checkEmbeddedScripts(file);
                errors.push(...scriptErrors);
            } catch (error) {
                logger.error('Script validation failed', { error, fileName: file.name });
            }
        }
    ...
    private async validateContent(file: File): Promise<{
        ...
        try {
            ...
            if (file.type === 'application/pdf' && this.config.content.validatePdfStructure) {
     ...
     */
    private async checkEmbeddedScripts(file: File): Promise<FileValidationError[]> {
        const errors: FileValidationError[] = [];
    ...
    private async validateImage(file: File): Promise<{
        ...
        const errors: FileValidationError[] = [];
        const warnings: FileValidationWarning[] = [];
        let metadata: Partial<FileMetadataExtract> = {};

        try {
            const dimensions = await this.getImageDimensions(file);
            metadata.dimensions = dimensions;

            if (dimensions.width > this.config.content.maxImageDimensions.width ||
            ...
            if (dimensions.width * dimensions.height > 25000000) { // 25 megapixels
                warnings.push({
     ...
     */
    private async extractFileMetadata(file: File): Promise<Partial<FileMetadataExtract>> {
        ...
        try {
            ...
            if (this.isTextBasedFile(file) && file.size < 1024 * 1024) { // Only for files < 1MB
    ...
    private async readFileAsText(file: File, encoding = 'utf-8', maxBytes?: number): Promise<string> {
        return new Promise((resolve, reject) => {
    ...
    private matchesSignature(header: Uint8Array, pattern: number[], offset: number): boolean {
...
export const createFileTypeRules = {
     ...
     */
    image: (maxMegapixels = 25): ValidationRule => async (file: File) => {
        const errors: FileValidationError[] = [];
