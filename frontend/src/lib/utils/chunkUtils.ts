 ...
 */
export function chunkFile(file: File, chunkSize: number = DEFAULT_CHUNK_SIZE): FileChunk[] {
    ...
    // Validate chunk size
    const validatedChunkSize = Math.max(MIN_CHUNK_SIZE, Math.min(MAX_CHUNK_SIZE, chunkSize));
    ...
    logger.debug('File chunked successfully', { fileName: file.name, chunks: chunks.length });
...
export function calculateProgress(
    ...
    const percentage = totalBytes > 0 ? Math.min(100, (uploadedBytes / totalBytes) * 100) : 0;
...
export async function createChunkHash(chunk: FileChunk): Promise<string> {
    try {
        ...
        logger.error('Failed to create chunk hash', { chunkIndex: chunk.index, error });
 ...
 */
export async function verifyChunk(chunk: FileChunk, expectedHash: string): Promise<boolean> {
    try {
        const actualHash = await createChunkHash(chunk);
        return actualHash === expectedHash;
    } catch (error) {
        logger.error('Chunk verification failed', { chunkIndex: chunk.index, error });
        return false;
    }
}
 ...
 */
export function shouldChunkFile(file: File, threshold: number = DEFAULT_CHUNK_SIZE): boolean {
