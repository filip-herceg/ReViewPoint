    ...
    try {
        ...
        for (const [group, extensions] of Object.entries(FILE_TYPE_GROUPS)) {
            if (extensions.includes(extension)) {
                logger.debug('File type group detected', { filename, extension, group });
                return group as FileTypeGroup;
            }
        }

        logger.debug('Unknown file type, defaulting to other', { filename, extension });
        return 'other';
    } catch (error) {
...
export function generateUniqueFilename(originalName: string, existingNames: string[] = []): string {
    try {
        ...
        logger.debug('Unique filename generated', { originalName, uniqueName, attempts: counter - 1 });
        return uniqueName;
    } catch (error) {
...
export function isPreviewSupported(filename: string, mimeType?: string): boolean {
    try {
        ...
        const isSupported = previewableExtensions.includes(extension) ||
            previewableMimeTypes.includes(type);

        logger.debug('Preview support check', { filename, extension, mimeType: type, isSupported });
        return isSupported;
    } catch (error) {
        logger.error('Error checking preview support', { error, filename, mimeType });
        return false;
    }
}
...
export async function generateFileHash(file: File): Promise<string> {
    try {
        logger.debug('Generating file hash', { filename: file.name, size: file.size });

        // Check if crypto.subtle is available
        if (!globalThis.crypto?.subtle) {
            logger.warn('crypto.subtle not available, using fallback hash', { filename: file.name });
            // Fallback to simple hash for environments without crypto.subtle
            const text = await readFileAsText(file);
            const simpleHash = Array.from(text).reduce((acc, char, index) =>
                ((acc << 5) - acc + char.charCodeAt(0) + index) & 0xffffffff, 0
...
export async function readFileAsText(file: File): Promise<string> {
    try {
        logger.debug('Reading file as text', { filename: file.name, size: file.size });
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = () => {
                ...
                logger.debug('File read as text', { filename: file.name, length: text.length });
                resolve(text);
            };
...
export async function readFileAsDataUrl(file: File): Promise<string> {
    try {
        logger.debug('Reading file as data URL', { filename: file.name, size: file.size });
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = () => {
                ...
                logger.debug('File read as data URL', { filename: file.name, dataUrlLength: dataUrl.length });
                resolve(dataUrl);
            };

            reader.onerror = () => {
                const error = new Error('Failed to read file as data URL');
                logger.error('Error reading file as data URL', { error, filename: file.name });
                reject(error);
            };
