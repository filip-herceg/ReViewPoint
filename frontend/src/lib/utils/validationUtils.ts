...
export interface FileValidation {
    ...
    customValidator?: (file: File) => Promise<ValidationResult> | ValidationResult;
}
...
export const FILE_TYPE_GROUPS = {
    images: ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml'] as string[],
    ...
    spreadsheets: ['application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'] as string[],
    presentations: ['application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation'] as string[],
    archives: ['application/zip', 'application/x-rar-compressed', 'application/x-7z-compressed'] as string[],
    text: ['text/plain', 'text/csv', 'text/html', 'text/css', 'text/javascript'] as string[],
 ...
 */
export function validateFile(file: File, validation: FileValidation = DEFAULT_VALIDATION): ValidationResult {
    const errors: string[] = [];
    ...
    try {
        ...
        if (validation.minSize && file.size < validation.minSize) {
            errors.push(`File size (${formatFileSize(file.size)}) is below minimum required size (${formatFileSize(validation.minSize)})`);
        }
        ...
        if (validation.allowedTypes && !validation.allowedTypes.includes('*')) {
            const isAllowed = validation.allowedTypes.some(allowedType => {
                if (allowedType.startsWith('.')) {
                    ...
                    const typeGroup = FILE_TYPE_GROUPS[allowedType as keyof typeof FILE_TYPE_GROUPS];
        ...
        if (validation.forbiddenTypes && validation.forbiddenTypes.length > 0) {
            const isForbidden = validation.forbiddenTypes.some(forbiddenType => {
                if (forbiddenType.startsWith('.')) {
                    ...
                    const typeGroup = FILE_TYPE_GROUPS[forbiddenType as keyof typeof FILE_TYPE_GROUPS];
                    return typeGroup ? typeGroup.includes(file.type) : false;
                }
            });
        ...
        logger.error('File validation failed with exception', { fileName: file.name, error });
        return {
            isValid: false,
 ...
 */
export function validateFiles(files: File[], validation: FileValidation = DEFAULT_VALIDATION): ValidationResult {
    ...
    if (validation.maxFiles && files.length > validation.maxFiles) {
        allErrors.push(`Too many files selected (${files.length}). Maximum allowed: ${validation.maxFiles}`);
    }

    if (validation.minFiles && files.length < validation.minFiles) {
        allErrors.push(`Not enough files selected (${files.length}). Minimum required: ${validation.minFiles}`);
    }
...
export async function validateFileContent(file: File): Promise<ValidationResult> {
    ...
    logger.debug('Starting content validation', { fileName: file.name, fileType: file.type });
    ...
    try {
        ...
        if (detectedType && file.type && !isCompatibleType(file.type, detectedType)) {
            warnings.push(`File type mismatch: declared as "${file.type}" but appears to be "${detectedType}"`);
        }
 ...
 * Validate image file
 */
async function validateImageFile(file: File, errors: string[], warnings: string[]): Promise<void> {
...
export function isDocumentFile(file: File): boolean {
    return FILE_TYPE_GROUPS.documents.includes(file.type) ||
