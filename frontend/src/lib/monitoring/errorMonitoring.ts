...
import { getEnvironmentConfig, getMonitoringConfig } from '@/lib/config/environment';
...
class ErrorMonitoringService {
    ...
    constructor() {
        // Configuration is loaded during initialize() to avoid module loading issues
    }
    ...
    private loadConfig(): ErrorMonitoringConfig {
        ...
        try {
            ...
            this.config = {
                ...
                maxErrors: 100,
                reportToSentry: !!monitoringConfig.sentryDsn && env.NODE_ENV === 'production',
            };
    ...
    private setupConsoleErrorTracking(): void {
        ...
        console.warn = (...args: any[]) => {
            ...
            if (!this.isExpectedWarning(message)) {
                ...
                if (this.originalConsole) {
                    this.originalConsole.debug('[DEBUG] Filtered expected warning:', message);
                }
            }
    ...
    private setupUnhandledRejectionTracking(): void {
        window.addEventListener('unhandledrejection', (event) => {
            ...
            const reasonString = typeof event.reason === 'string' ? event.reason : String(event.reason);
        ...
        window.addEventListener('error', (event) => {
            // Ignore source map related errors
            if (this.isSourceMapError(event.error) || this.isSourceMapError(event.message)) {
                return;
            }
    ...
    captureError(
        ...
        try {
            ...
            // Use original console to avoid recursion loop
            if (this.originalConsole) {
                this.originalConsole.error('[ERROR] Failed to process error', processingError);
            } else {
    ...
    private storeError(errorReport: ErrorReport): void {
        ...
        try {
            ...
            if (this.originalConsole) {
                this.originalConsole.warn('[WARN] Failed to store errors in localStorage', error);
            } else {
    ...
    private reportError(errorReport: ErrorReport): void {
        const config = this.loadConfig();
        if (config.reportToSentry && typeof window !== 'undefined' && (window as any).Sentry) {
            try {
                ...
                // Use original console to avoid recursion
                if (this.originalConsole) {
                    this.originalConsole.warn('[WARN] Failed to report error to Sentry', error);
                } else {
    ...
    clearErrors(): void {
        ...
        try {
            ...
            // Use original console to avoid recursion
            if (this.originalConsole) {
                this.originalConsole.warn('[WARN] Failed to clear errors from localStorage', error);
            } else {
...
export function createErrorFallback() {
    return function ErrorFallback({ error, resetErrorBoundary }: FallbackProps) {
        ...
        return React.createElement(
            'div',
            { className: 'error-fallback p-6 bg-red-50 border border-red-200 rounded-lg' },
            React.createElement(
                'h2',
            ...
            React.createElement(
                ...
                React.createElement(
                    'pre',
                    { className: 'text-sm text-red-700 bg-red-100 p-2 rounded overflow-auto' },
                    error.message
                )
            ),
            React.createElement(
...
export function useErrorMonitoring() {
    return {
        captureError: errorMonitoringService.captureError.bind(errorMonitoringService),
        getErrors: errorMonitoringService.getErrors.bind(errorMonitoringService),
        getErrorStats: errorMonitoringService.getErrorStats.bind(errorMonitoringService),
        clearErrors: errorMonitoringService.clearErrors.bind(errorMonitoringService),
        reinitialize: errorMonitoringService.reinitialize.bind(errorMonitoringService),
    };
}
