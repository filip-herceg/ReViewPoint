...
const UploadList: React.FC<UploadListProps> = ({ disableAutoFetch }) => {
  const { uploads, loading, error, fetchUploads, patchUpload, removeUpload } =
    useUploadStore();
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      ...
      const errorObj =
        err instanceof Error ? err : createTestError("Patch failed");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Use createTestError for demonstration/testing if needed
      const errorObj =
        err instanceof Error ? err : createTestError("Delete failed");
      setLocalError(getErrorMessage(errorObj));
    }
  };
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <div className="flex items-center gap-1">
                        <Badge
                          variant={
                            u.status === "pending" ? "secondary" : "default"
                          }
                        >
                          {u.status}
                        </Badge>
