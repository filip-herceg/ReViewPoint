...
import type { UploadQueueItem } from "@/lib/api/types/upload";
	...
	return (
		...
		>
			<div className="flex items-center gap-3">
					...
					{showDetails && !compact && (
						<div className="mt-1 space-y-1">
							<div className="flex items-center gap-4 text-xs text-muted-foreground">
								...
								{item.priority !== 5 && <span>Priority: {item.priority}</span>}
								{uploadDuration && (
					...
					{showProgress && item.status === "uploading" && (
						<div className="mt-2 space-y-1">
							...
							<div className="flex justify-between text-xs text-muted-foreground">
								...
								<span>
									{formatFileSize(item.progress.bytesTransferred)} /{" "}
				...
				{/* Status Badge */}
				<div className="flex-shrink-0">
					<Badge variant={getStatusBadgeVariant()}>{item.status}</Badge>
				</div>
...
export const UploadQueue: React.FC<UploadQueueProps> = ({
	...
	const handleRemove = useCallback(
		(item: UploadQueueItem) => {
			logger.info("Removing item from queue", {
				fileId: item.id,
				fileName: item.file.name,
			});
		...
		if (newPausedState) {
			...
			queue.forEach((item) => {
				if (item.status === "cancelled") {
					// Using cancelled as paused state
					resumeQueueItem(item.id);
				}
			});
