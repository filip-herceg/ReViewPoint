 ...
 */
export interface FileValidationFeedbackProps extends FileValidationFeedbackConfig {
...
const ValidationIssue: React.FC<ValidationIssueProps> = ({
    ...
    const handleDismiss = () => {
        ...
        logger.debug('Validation issue dismissed', { type, code: issue.code, index });
    };
...
const FileMetadata: React.FC<FileMetadataProps> = ({
    ...
    const basicInfo = [
        ...
        { icon: <HardDrive className="h-4 w-4" />, label: 'Size', value: formatFileSize(file.size) },
        { icon: <Zap className="h-4 w-4" />, label: 'Type', value: file.type || 'Unknown' },
        { icon: <Info className="h-4 w-4" />, label: 'Modified', value: formatDate(file.lastModified) }
    ];
...
export const FileValidationFeedback: React.FC<FileValidationFeedbackProps> = ({
    ...
    const groupedIssues = useMemo(() => {
        ...
        const groupBy = (items: (FileValidationError | FileValidationWarning)[], keyFn: (item: any) => string) => {
        ...
        return {
            errors: groupBy(validation.errors, (error) => error.code || error.message),
            warnings: groupBy(validation.warnings, (warning) => warning.code || warning.message)
        };
    ...
    const hiddenErrorsCount = Math.max(0, groupedIssues.errors.length - maxIssues);
    const hiddenWarningsCount = Math.max(0, groupedIssues.warnings.length - maxIssues);
        ...
        <div className={cn(
            ...
            {showProgress && (
                <div className="space-y-2">
                    ...
                    <Progress
                        value={validation.isValid ? 100 : 75}
                        className="h-2"
                    />
                </div>
            )}
