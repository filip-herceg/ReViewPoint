...
function SkipLink({ href, children, className }: SkipLinkProps) {
	const handleClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
		...
		if (target instanceof HTMLElement) {
			...
			logger.info("SkipLink: Navigated to target", {
				href,
...
export function EnhancedSkipLinks({ className }: { className?: string }) {
	...
	React.useEffect(() => {
		const discoverLinks = () => {
			...
			const landmarks = [
				{
					selector: 'main, [role="main"], #main, #main-content',
					label: "Skip to main content",
				},
				{
					selector: 'nav, [role="navigation"], #navigation',
					label: "Skip to navigation",
				},
				{
					selector: 'aside, [role="complementary"], #sidebar',
			...
			landmarks.forEach((landmark) => {
				...
				if (element) {
					...
					if (!element.id) {
						const generatedId = landmark.selector
							.split(",")[0]
							.replace(/[^\w-]/g, "")
