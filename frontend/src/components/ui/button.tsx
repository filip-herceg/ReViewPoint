...
const buttonVariantClasses: Record<string, string> = {
  ...
  destructive: "bg-destructive text-[var(--destructive-foreground)] shadow-xs hover:bg-destructive/90 focus-visible:ring-destructive/20 dark:focus-visible:ring-destructive/40 dark:bg-destructive/60",
  ...
  "outline-destructive": "border border-destructive text-destructive bg-background shadow-xs hover:bg-destructive/10 focus-visible:ring-destructive/20",
  "outline-success": "border border-[var(--color-success)] text-[var(--color-success)] bg-background shadow-xs hover:bg-[var(--color-success)]/10 focus-visible:ring-[var(--color-success)]/20",
  secondary: "bg-secondary text-secondary-foreground shadow-xs hover:bg-secondary/80",
  ghost: "hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50",
  link: "text-primary underline-offset-4 hover:underline bg-transparent shadow-none",
  "icon-sm": "hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 focus:outline-none",
  ...
  "theme-option": "p-3 border rounded-lg flex flex-col items-center gap-2 transition-colors border-border hover:bg-muted data-[active=true]:border-primary data-[active=true]:bg-primary/10",
  "floating-icon": "absolute bg-primary text-[var(--text-inverse)] rounded-full hover:opacity-90 transition-all shadow-md",
};
...
function Button({
  ...
  if (asChild) {
    ...
    if (!React.isValidElement(firstChild)) {
      if (process.env.NODE_ENV === 'development') {
        console.warn('Button: asChild prop requires the first child to be a React element');
      }
      return null;
    }
