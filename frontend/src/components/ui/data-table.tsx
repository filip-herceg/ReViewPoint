...
export function DataTable<T extends Record<string, any>>({
    ...
    const handleRowSelection = (index: number, selected: boolean) => {
        ...
        try {
            ...
            logger.debug('DataTable selection changed', { index, selected, total: newSelection.size });
        } catch (err) {
            logger.error('Failed to handle row selection', { index, selected, error: err });
        }
    };
    ...
    const handleSelectAll = (selected: boolean) => {
        ...
        try {
            ...
            selection.onSelectionChange(newSelection);
            logger.debug('DataTable select all changed', { selected, total: newSelection.size });
        } catch (err) {
    ...
    const allSelectableRows = data.filter((row, index) =>
        !selection?.selectable || selection.selectable(row, index)
    );
    const selectedCount = selection?.selectedRows.size || 0;
    const allSelected = selectedCount > 0 && selectedCount === allSelectableRows.length;
    const someSelected = selectedCount > 0 && selectedCount < allSelectableRows.length;
    ...
    return (
        <div className={cn('space-y-4', className)} data-testid={testId}>
                ...
                <table className="w-full" data-testid={`${testId}-table`}>
                    ...
                    <tbody>
                        {data.length === 0 ? (
                            ...
                            data.map((row, index) => {
                                const isSelectable = !selection?.selectable || selection.selectable(row, index);
            ...
            {/* Pagination */}
            {pagination && (
                <div className="flex items-center justify-between" data-testid={`${testId}-pagination`}>
                    <div className="flex items-center space-x-2">
