...
const validateFile = (file: File, validation: FileValidation): string | null => {
    ...
    // Check file type
    if (validation.allowedTypes && validation.allowedTypes.length > 0 && !validation.allowedTypes.includes('*')) {
...
export function FileUpload({
    ...
    const handleFiles = useCallback((selectedFiles: FileList | File[]) => {
        try {
            ...
            if (mergedValidation.maxFiles && totalFiles > mergedValidation.maxFiles) {
    ...
    }, [files, mergedValidation, multiple, onFilesSelected, autoUpload, onUpload]);
    ...
    return (
        <div className={cn('space-y-4', className)} data-testid={testId}>
            ...
            <div
                ...
                onDrop={handleDrop}
                onClick={() => !disabled && document.getElementById(`file-input-${testId}`)?.click()}
            ...
            >
                ...
                <div className="mt-4 text-xs text-muted-foreground space-y-1">
                    ...
                    {mergedValidation.allowedTypes && mergedValidation.allowedTypes.length > 0 && !mergedValidation.allowedTypes.includes('*') && (
                        <div>Allowed types: {mergedValidation.allowedTypes.join(', ')}</div>
                    )}
                ...
                <div className="space-y-2" data-testid={`${testId}-preview`}>
                    ...
                    {files.length > 0 && (
                        <div className="text-sm text-muted-foreground flex space-x-4">
                            ...
                            {uploadingFiles.length > 0 && <span>Uploading: {uploadingFiles.length}</span>}
                            {successFiles.length > 0 && <span>Success: {successFiles.length}</span>}
                            {errorFiles.length > 0 && <span>Errors: {errorFiles.length}</span>}
                        </div>
                    )}
                </div>
            )}
