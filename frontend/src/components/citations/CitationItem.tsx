...
const formatCitation = (citation: Citation, format: string = 'APA'): string => {
    const authorsText = citation.authors.length > 0 ? citation.authors.join(', ') : 'Unknown Author';
...
export const CitationItem: React.FC<CitationItemProps> = ({ 
    ...
    const handleCopyFormatted = () => {
        const formatted = formatCitation(citation, documentCitation?.citationFormat);
                ...
                <div className="space-y-3">
                    {(citation.volume || citation.issue || citation.pages) && (
                        <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            ...
                            {citation.issue && (
                                <span>Issue {citation.issue}</span>
                            )}
                            {citation.pages && (
                                <span>Pages {citation.pages}</span>
                            )}
                        </div>
                    )}
