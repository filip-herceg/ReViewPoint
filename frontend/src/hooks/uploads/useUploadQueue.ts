...
import type { UploadQueueItem, UploadQueueConfig } from '@/lib/api/types/upload';
...
export function useUploadQueue(config: Partial<UploadQueueConfig> = {}) {
    ...
    const addToQueue = useCallback((item: UploadQueueItem) => {
        ...
        setQueue(currentQueue => {
            ...
            const insertIndex = newQueue.findIndex(queueItem => queueItem.priority < item.priority);
     ...
     */
    const getQueueItem = useCallback((id: string): UploadQueueItem | undefined => {
     ...
     */
    const getQueueItemsByStatus = useCallback((status: UploadQueueItem['status']): UploadQueueItem[] => {
    ...
    const moveQueueItem = useCallback((id: string, newIndex: number) => {
        setQueue(currentQueue => {
            ...
            if (itemIndex === -1 || newIndex < 0 || newIndex >= currentQueue.length) {
                logger.warn('Cannot move queue item - invalid index', { id, itemIndex, newIndex });
                return currentQueue;
            }
     ...
     */
    const changePriority = useCallback((id: string, newPriority: number) => {
        updateQueueItem(id, { priority: newPriority });

        // Re-sort queue based on new priority
        setQueue(currentQueue => {
            const sorted = [...currentQueue].sort((a, b) => b.priority - a.priority);
    ...
    const retryFailedItems = useCallback(() => {
        ...
        failedItems.forEach(item => {
            if (item.error && item.error.retryable && item.error.retryCount < defaultConfig.maxRetries) {
    ...
    const processQueue = useCallback(async () => {
        ...
        try {
            ...
            const availableSlots = defaultConfig.maxConcurrent - uploadingItems.length;
