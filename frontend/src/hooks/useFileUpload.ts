...
export function useFileUpload(options: UseFileUploadOptions = {}) {
    ...
    const updateUploadProgress = useCallback((uploadId: string, progress: number, additionalData: Partial<FileUploadInfo> = {}) => {
        setState(prevState => {
            const uploads = new Map(prevState.uploads);
            const upload = uploads.get(uploadId);

            if (!upload) {
                logger.warn('Attempted to update progress for non-existent upload', { uploadId });
                return prevState;
            }
            ...
            logger.debug('Updated upload progress', { uploadId, progress, status: updatedUpload.status });
    ...
    const uploadFile = useCallback(async (file: File, uploadOptions: UploadOptions = {}): Promise<string> => {
        const uploadId = generateUploadId();

        logger.info('Starting file upload', { uploadId, fileName: file.name, fileSize: file.size });
        ...
        try {
            ...
            logger.info('File upload completed successfully', { uploadId, fileName: file.name });
            ...
            logger.error('File upload failed', { uploadId, fileName: file.name, error: errorMessage });
            ...
            onError?.(uploadId, error instanceof Error ? error : new Error(errorMessage));
            throw error;
        } finally {
            // Clean up abort controller
            abortControllersRef.current.delete(uploadId);
        }
    }, [validation, enableChunked, chunkSize, updateUploadProgress, addUpload, onComplete, onError]);

    // Simple upload for small files
    const uploadFileSimple = useCallback(async (
        uploadId: string,
        file: File,
        options: UploadOptions & { signal?: AbortSignal }
    ): Promise<UploadResult> => {
        logger.debug('Starting simple file upload', { uploadId, fileName: file.name });
    ...
    const uploadFileChunked = useCallback(async (
        ...
        logger.debug('Starting chunked file upload', { uploadId, fileName: file.name, fileSize: file.size });
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            try {
                logger.debug('Uploading chunk', { uploadId, chunkIndex: i, totalChunks });

                // Update chunk status
                chunkInfo.status = 'uploading';
                updateUploadProgress(uploadId, (i / totalChunks) * 100, { chunks: chunkInfos });
                ...
                logger.error('Chunk upload failed', { uploadId, chunkIndex: i, error });
    ...
    const retryUpload = useCallback(async (uploadId: string): Promise<void> => {
        ...
        if (!upload || upload.status !== 'error') {
            logger.warn('Cannot retry upload: invalid upload or status', { uploadId, status: upload?.status });
            return;
        }
    ...
    const clearCompleted = useCallback(() => {
        setState(prevState => {
            ...
            logger.debug('Cleared completed uploads', { remainingUploads: uploads.size });
    ...
    return {
        ...
        getUploadsByStatus: (status: FileUploadInfo['status']) =>
            Array.from(state.uploads.values()).filter(upload => upload.status === status),
    };
}
