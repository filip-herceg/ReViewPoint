...
export function renderWithErrorBoundary(ui: React.ReactElement, onError?: (error: Error) => void) {
    class TestErrorBoundary extends React.Component<{ children: React.ReactNode }, { error: Error | null }> {
        ...
        render(): React.ReactNode {
            if (this.state.error) {
                ...
                return (
                    React.createElement('div', { 'data-testid': 'test-error-boundary' }, `Error: ${getErrorMessage(this.state.error)}`)
                );
            }
...
export function deepClone<T>(obj: T): T {
    ...
    } else if (typeof obj === 'object' && obj !== null && Object.keys(obj).length > 20) {
        testLogger.warn('Deep cloned large object:', { keys: Object.keys(obj).length });
    } else {
...
export function merge<T, U>(a: T, b: U): T & U {
    ...
    } else if (typeof merged === 'object' && merged !== null && Object.keys(merged).length > 20) {
