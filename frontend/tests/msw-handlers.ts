...
export const handlers = [
    ...
    http.post('/api/uploads', async ({ request }) => {
        ...
        const data = createUpload({ id: '2', name: 'new.pdf', status: 'pending', progress: 0 });
    ...
    http.patch('/api/uploads/:id', async ({ request, params }) => {
        ...
        const data = createUpload({ id: '1', name: 'file.pdf', status: 'completed', progress: 100 });
        logger.info('Responding with:', data);
        return new Response(
            JSON.stringify(data),
            { status: 200, headers: { 'Content-Type': 'application/json' } }
    ...
    http.all(/\/api\/uploads.*/, ({ request }) => {
        ...
        return HttpResponse.json({ error: 'MSW fallback: Not found' }, { status: 404 });
    }),
