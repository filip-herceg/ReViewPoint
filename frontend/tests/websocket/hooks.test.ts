...
import { createTestNotification, createTestUploadProgress, createTestConnectionMetadata } from '../test-templates';
...
describe('WebSocket Hooks', () => {
    ...
    describe('useWebSocketNotifications', () => {
        ...
        it('should remove all read notifications', () => {
            const notification1 = createTestNotification({ read: true, persistent: false });
            const notification2 = createTestNotification({ read: false });
            const notification3 = createTestNotification({ read: true, persistent: false });
    ...
    describe('useWebSocketUpload', () => {
        it('should return upload progress', () => {
            ...
            const progressData = createTestUploadProgress({ uploadId, progress: 75, status: 'uploading' });
        ...
        it('should subscribe to upload events', () => {
            ...
            expect(webSocketService.on).toHaveBeenCalledWith('upload.progress', expect.any(Function));
            expect(webSocketService.on).toHaveBeenCalledWith('upload.completed', expect.any(Function));
            expect(webSocketService.on).toHaveBeenCalledWith('upload.error', expect.any(Function));
        });
    });
    ...
    describe('useWebSocketEvent', () => {
        ...
        it('should unsubscribe on unmount', () => {
            ...
            const { unmount } = renderHook(() => useWebSocketEvent(event, mockHandler));
    ...
    describe('hook integration', () => {
        it('should work together with multiple hooks', () => {
            ...
            const { result: notificationResult } = renderHook(() => useWebSocketNotifications());
            const { result: uploadResult } = renderHook(() => useWebSocketUpload('test-upload'));
