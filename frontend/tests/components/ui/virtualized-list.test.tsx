...
describe('VirtualizedList Component', () => {
    ...
    it('uses custom key extractor', () => {
        ...
        const keyExtractor = vi.fn((item, index) => `custom-${item.id}`);
    ...
    it('calculates total height correctly', () => {
        ...
        const totalHeightContainer = container.querySelector('div[style*="position: relative"]');
...
describe('VirtualizedGrid Component', () => {
    ...
    it('positions items correctly', () => {
        ...
        const itemsContainer = container.querySelector('div[style*="position: absolute"][style*="transform"]');
        expect(itemsContainer).toBeInTheDocument();

        // Check that items inside have position styling applied directly
        const items = itemsContainer?.querySelectorAll('div[style*="position: absolute"]');
...
describe('useVirtualizedList Hook', () => {
    const TestComponent = ({ items }: { items: any[] }) => {
        const { scrollTop, scrollToIndex, scrollToTop, scrollToBottom, setScrollTop } = useVirtualizedList(items, {
            itemHeight: 50,
            containerHeight: 200
        });
    ...
    it('handles edge cases', () => {
        ...
        const scrollTop = parseInt(screen.getByTestId('scroll-top').textContent || '0');
