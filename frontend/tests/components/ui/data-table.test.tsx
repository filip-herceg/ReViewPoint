...
const defaultColumns: DataTableColumn<TestUser>[] = [
    ...
    { key: 'status', title: 'Status', render: (value) => <span className={value === 'active' ? 'text-green-600' : 'text-red-600'}>{value}</span> },
];
...
describe('DataTable Component', () => {
    ...
    it('renders table with data correctly', () => {
        ...
        // Check cell content
        expect(screen.getByTestId('test-table-cell-0-name')).toHaveTextContent('John Doe');
        expect(screen.getByTestId('test-table-cell-1-email')).toHaveTextContent('jane@example.com');
    });
    ...
    it('shows empty state when no data', () => {
        render(<DataTable {...defaultProps} data={[]} emptyMessage="No users found" />);
        ...
        const customColumns: DataTableColumn<TestUser>[] = [
            {
                ...
                title: 'Status',
                render: (value, row) => (
                    <div data-testid={`status-${row.id}`} className={value === 'active' ? 'text-green-600' : 'text-red-600'}>
                        {value.toUpperCase()}
                    </div>
                ),
            },
