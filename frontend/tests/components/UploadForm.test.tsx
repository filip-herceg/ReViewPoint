...
function setupUseUploadStoreMock(storeOverrides: Partial<ReturnType<typeof uploadStoreModule.useUploadStore>> = {}) {
    const store = {
        createUpload: vi.fn(),
        loading: false,
        error: null,
    ...
    useUploadStoreSpy = vi.spyOn(uploadStoreModule, 'useUploadStore').mockReturnValue(store as any);
    return store;
}

// Utility to render UploadForm with custom store values
function renderUploadFormWithStore(storeOverrides: Partial<ReturnType<typeof uploadStoreModule.useUploadStore>>) {
...
describe('UploadForm', () => {
    ...
    it('renders and submits form data', async () => {
        ...
        expect(store.createUpload).toHaveBeenCalledWith({ name: formData.name, status: 'pending', progress: 0 });
    });
    it('shows error from store', () => {
        renderUploadFormWithStore({ error: { type: 'unknown', message: 'Something went wrong', original: 'test' } });
    ...
    it('disables submit button when loading', () => {
        ...
        testLogger.info('Checked button disabled state', { disabled: button.disabled });
    ...
    it('clears local error after successful submit', async () => {
        const store = setupUseUploadStoreMock({
            createUpload: vi.fn().mockRejectedValueOnce(createTestError('fail')).mockResolvedValueOnce(undefined),
        });
        render(<UploadForm />);
        const input = screen.getByLabelText(/file name/i);
        fireEvent.change(input, { target: { value: 'fail.pdf' } });
