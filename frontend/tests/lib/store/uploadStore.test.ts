...
describe('uploadStore (sync actions)', () => {
    ...
    it('should add an upload', () => {
        ...
        testLogger.debug('Uploads after addUpload', useUploadStore.getState().uploads)
    ...
    it('should update an upload', () => {
        ...
        const upload = createUpload({ status: 'pending', progress: 0 })
        useUploadStore.getState().addUpload(upload)
        useUploadStore.getState().updateUpload(upload.id, { status: 'completed', progress: 100 })
        const updated = useUploadStore.getState().uploads.find(u => u.id === upload.id)
    ...
    it('should set current upload', () => {
        ...
        testLogger.debug('Current upload after setCurrentUpload(upload)', useUploadStore.getState().currentUpload)
        expect(useUploadStore.getState().currentUpload).toEqual(upload)
        useUploadStore.getState().setCurrentUpload(null)
        testLogger.debug('Current upload after setCurrentUpload(null)', useUploadStore.getState().currentUpload)
