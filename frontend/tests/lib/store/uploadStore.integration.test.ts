...
describe('uploadStore async actions', () => {
    ...
    it('createUpload adds upload to state', async () => {
        ...
        getMockedRequest().mockResolvedValueOnce({ data: upload, status: 200, statusText: 'OK', headers: {}, config: {} })
        await useUploadStore.getState().createUpload({ name: upload.name, status: upload.status, progress: upload.progress })
    ...
    it('patchUpload updates upload in state', async () => {
        ...
        getMockedRequest().mockResolvedValueOnce({ data: updated, status: 200, statusText: 'OK', headers: {}, config: {} })
        await useUploadStore.getState().patchUpload(orig.id, { status: 'completed', progress: 100 })
    ...
    it('removeUpload removes upload from state', async () => {
        ...
        const orig = createUpload({ status: 'pending', progress: 0 })
        useUploadStore.setState({ uploads: [orig] })
        getMockedRequest().mockResolvedValueOnce({ data: null, status: 200, statusText: 'OK', headers: {}, config: {} })
