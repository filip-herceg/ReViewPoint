...
const getMockedRequest = () => (mockedAxios.request as unknown as ReturnType<typeof vi.fn>);
...
describe('uploadStore async actions', () => {
    ...
    it('fetchUploads sets uploads and loading', async () => {
        ...
        testLogger.debug('Loading after fetchUploads called', useUploadStore.getState().loading)
        expect(useUploadStore.getState().loading).toBe(true);
        await promise;
        testLogger.debug('Uploads after fetchUploads', useUploadStore.getState().uploads)
    ...
    it('createUpload adds upload and sets loading', async () => {
        ...
        testLogger.debug('Loading after createUpload called', useUploadStore.getState().loading)
        expect(useUploadStore.getState().loading).toBe(true);
        await promise;
        testLogger.debug('Uploads after createUpload', useUploadStore.getState().uploads)
    ...
    it('patchUpload updates upload and sets loading', async () => {
        ...
        const promise = useUploadStore.getState().patchUpload(orig.id, { status: 'completed', progress: 100 });
        testLogger.debug('Loading after patchUpload called', useUploadStore.getState().loading)
        expect(useUploadStore.getState().loading).toBe(true);
        await promise;
        testLogger.debug('Uploads after patchUpload', useUploadStore.getState().uploads)
        expect(useUploadStore.getState().uploads[0]).toMatchObject({ status: 'completed', progress: 100 });
        expect(useUploadStore.getState().loading).toBe(false);
        expect(useUploadStore.getState().error).toBeNull();
    });
    ...
    it('removeUpload removes upload and sets loading', async () => {
        ...
        testLogger.debug('Loading after removeUpload called', useUploadStore.getState().loading)
        expect(useUploadStore.getState().loading).toBe(true);
        await promise;
        testLogger.debug('Uploads after removeUpload', useUploadStore.getState().uploads)
    ...
    it('sets error on API failure', async () => {
        ...
        testLogger.debug('Error after failed fetchUploads', useUploadStore.getState().error)
