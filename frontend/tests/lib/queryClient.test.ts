...
describe('queryClient error handling config', () => {
    ...
    it('can subscribe to global query errors via queryCache', async () => {
        ...
        const query = queryClient.getQueryCache().build(queryClient, {
            queryKey: ['test'],
            queryFn: async () => { throw new Error('global error'); },
        });
    ...
    it('can subscribe to global mutation errors via mutationCache', async () => {
        ...
        const mutation = queryClient.getMutationCache().build(queryClient, {
            ...
            mutationFn: async () => { throw new Error('mutation error'); },
        });
        try {
    ...
    it('handles advanced error normalization scenarios', () => {
        ...
        // Axios-like error
        const axiosError = { isAxiosError: true, response: { status: 500, data: { error: 'fail' } } };
    ...
    it('invokes global queryCache and mutationCache listeners and handles errors', async () => {
        ...
        // Register listeners
        const unsubscribeQuery = queryClient.getQueryCache().subscribe(queryListener);
        const unsubscribeMutation = queryClient.getMutationCache().subscribe(mutationListener);

        // Simulate a query error by building a query that throws
        const query = queryClient.getQueryCache().build(queryClient, {
            queryKey: ['test-listener'],
            queryFn: async () => { throw createTestError('query fail'); },
        });
        try {
            await query.fetch();
        } catch { }
        ...
        const mutation = queryClient.getMutationCache().build(queryClient, {
            mutationFn: async () => { throw createTestError('mutation fail'); },
        });
        try {
            await mutation.execute(undefined);
        } catch { }
        ...
        expect(queryEvents.some(e => e?.type === 'updated' && e.action?.type === 'error')).toBe(true);
        // For mutation, look for event.type === 'updated' && e.action.type === 'error'
        expect(mutationEvents.some(e => e?.type === 'updated' && e.action?.type === 'error')).toBe(true);
    });
