...
describe('AuthErrorType', () => {
    it('should have all expected error types', () => {
        ...
        expect(AuthErrorType.REFRESH_TOKEN_BLACKLISTED).toBe('refresh_token_blacklisted');
        expect(AuthErrorType.REFRESH_TOKEN_RATE_LIMITED).toBe('refresh_token_rate_limited');
...
describe('JWT utilities', () => {
    ...
    const mockToken = [
        ...
        btoa(JSON.stringify(mockPayload)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''), // payload
        'signature', // signature
    ].join('.');
    ...
    describe('isTokenExpired', () => {
        ...
        it('should detect expired tokens', () => {
            ...
            const expiredToken = [
                ...
                btoa(JSON.stringify(expiredPayload)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''),
                'signature',
            ].join('.');
    ...
    describe('extractUserFromToken', () => {
        ...
        it('should return null for invalid tokens', () => {
            ...
            const invalidTokens: AuthTokens = { access_token: 'invalid', refresh_token: 'invalid', token_type: 'bearer' };
            expect(extractUserFromToken(invalidTokens)).toBeNull();

            const emptyTokens: AuthTokens = { access_token: '', refresh_token: '', token_type: 'bearer' };
            ...
            const incompleteToken = [
                'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9',
                btoa(JSON.stringify(incompletePayload)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''),
                'signature',
            ].join('.');
