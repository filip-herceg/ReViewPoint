...
describe('Uploads API', () => {
    ...
    describe('Test and diagnostic endpoints', () => {
        it('should test root endpoint successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Testing uploads root endpoint');
        ...
        it('should test alive endpoint successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Testing uploads alive endpoint');
        ...
        it('should test export alive endpoint successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Testing uploads export alive endpoint');
    ...
    describe('exportFiles', () => {
        it('should export files as CSV successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Exporting files as CSV', { params: undefined });
            expect(mockLogger.info).toHaveBeenCalledWith('Files exported successfully');
            expect(result).toBe(csvData);
        });

        it('should export files with parameters', async () => {
            const params = { q: 'test', sort: 'filename' as const, order: 'asc' as const };
            const csvData = 'filename,url\ntest.pdf,/uploads/test.pdf\n';
            mockRequest.mockResolvedValue({ data: csvData });

            const result = await uploadsApi.exportFiles(params);

            expect(mockRequest).toHaveBeenCalledWith('/uploads/export?q=test&sort=filename&order=asc', {
                headers: {
                    'Accept': 'text/csv'
                }
            });
    ...
    describe('uploadFile', () => {
        ...
        it('should upload file successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('File uploaded successfully', {
                filename: 'test.pdf',
        ...
        it('should handle upload error', async () => {
            ...
            await expect(uploadsApi.uploadFile(mockFile)).rejects.toThrow(errorMessage);
    ...
    describe('getFiles', () => {
        it('should fetch files list successfully', async () => {
            ...
            expect(mockRequest).toHaveBeenCalledWith('/uploads');
            expect(mockLogger.info).toHaveBeenCalledWith('Fetching files list', { params: undefined });
            expect(mockLogger.info).toHaveBeenCalledWith('Files fetched successfully', {
                total: 2,
                count: 2
            });
            expect(result).toEqual(mockResponse);
        ...
        it('should fetch files with parameters', async () => {
            ...
            expect(mockRequest).toHaveBeenCalledWith('/uploads?q=test&limit=10&offset=0');
            expect(result).toEqual(mockResponse);
        });
    ...
    describe('getFileByFilename', () => {
        it('should fetch file info successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Fetching file info', { filename });
            expect(mockLogger.info).toHaveBeenCalledWith('File info fetched successfully', { 
                filename: 'test.pdf' 
            });
            expect(result).toEqual(mockResponse);
        });
        ...
        it('should handle file not found', async () => {
            ...
            await expect(uploadsApi.getFileByFilename(filename)).rejects.toThrow(errorMessage);

            expect(mockLogger.warn).toHaveBeenCalledWith('Failed to fetch file info', { 
                error: errorMessage,
                filename
            });
        });
        ...
        it('should encode filename properly', async () => {
            ...
            const mockResponse = { filename, url: '/uploads/file%20with%20spaces.pdf' };
            mockRequest.mockResolvedValue({ data: mockResponse });

            await uploadsApi.getFileByFilename(filename);

            expect(mockRequest).toHaveBeenCalledWith('/uploads/file%20with%20spaces.pdf');
        });
    });
    ...
    describe('deleteFile', () => {
        it('should delete file successfully', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Deleting file', { filename });
            expect(mockLogger.info).toHaveBeenCalledWith('File deleted successfully', { filename });
        });
        ...
        it('should handle deletion error', async () => {
            ...
            await expect(uploadsApi.deleteFile(filename)).rejects.toThrow(errorMessage);
