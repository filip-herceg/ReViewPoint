...
describe('upload.queries', () => {
    ...
    beforeEach(() => {
        testLogger.info('Resetting handlers, QueryClient, clearing mocks, and clearing react-query cache before test');
    ...
    describe('fetches uploads (useUploads)', () => {
        it('should return uploads array', async () => {
            ...
            const { result } = renderHook(() => uploadQueries.useUploads(), { wrapper: createWrapper() });
            await waitFor(() => result.current.status === 'success', { timeout: 2000 });
            ...
            if (data.length === 0) {
                ...
                await waitFor(() => Array.isArray(result.current.data) && result.current.data.length === 1, { timeout: 2000 });
            }
    ...
    describe('handles error in useUploads', () => {
        it('should return error', async () => {
            ...
            if (result.current.error == null && await result.current.refetch()) {
                ...
                await waitFor(() => result.current.status === 'error' && result.current.error != null, { timeout: 3000 });
            }
    ...
    describe('fetches single upload (useUpload)', () => {
        it('should return single upload', async () => {
            ...
            const { result } = renderHook(() => uploadQueries.useUpload('1'), { wrapper: createWrapper() });
            await waitFor(() => result.current.status === 'success', { timeout: 2000 });
            ...
            if (!result.current.data) {
                testLogger.warn('Single upload data was undefined, refetching and waiting again');
                await act(async () => {
                    await result.current.refetch();
                });
                await waitFor(() => result.current.status === 'success' && result.current.data && result.current.data.filename === 'file.pdf', { timeout: 2000 });
            }
    ...
    it('createUpload mutation works', async () => {
        ...
        vi.spyOn(uploadsApi, 'uploadFile').mockResolvedValueOnce({ filename: 'new.pdf', url: '/uploads/new.pdf' });
        const uploadQueries = await import('@/lib/api/upload.queries');
        const { result } = renderHook(() => uploadQueries.useCreateUpload(), { wrapper: createWrapper() });
        await act(async () => {
            const file = new File(['test content'], 'new.pdf', { type: 'application/pdf' });
    ...
    it('updateUpload mutation throws error', async () => {
        ...
        const uploadQueries = await import('@/lib/api/upload.queries');
        const { result } = renderHook(() => uploadQueries.useUpdateUpload(), { wrapper: createWrapper() });
        await act(async () => {
            try {
                await result.current.mutateAsync({ id: '1', data: { status: 'completed', progress: 100 } });
                expect.fail('Should have thrown an error');
            } catch (error: any) {
                testLogger.debug('updateUpload mutation error', error)
                expect(error.message).toBe('File updates are not supported. Please delete and re-upload the file.');
            }
        });
    });
    ...
    it('deleteUpload mutation works', async () => {
        ...
        const uploadQueries = await import('@/lib/api/upload.queries');
        const { result } = renderHook(() => uploadQueries.useDeleteUpload(), { wrapper: createWrapper() });
        await act(async () => {
