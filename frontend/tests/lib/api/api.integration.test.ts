...
import type { User, FileListItem } from '@/lib/api/types';
import axios from 'axios';
import { createUser, createUpload, createUploadList } from '../../test-templates';
...
describe('api utility', () => {
    it('should handle successful login', async () => {
        ...
        const authResponse = { access_token: 'token123', refresh_token: 'refresh123' };
        getMockedRequest().mockResolvedValueOnce({ data: authResponse });

        const res = await authApi.login({ email: 'test@example.com', password: 'pw' });
    ...
    it('should handle login error (object)', async () => {
        ...
        getMockedRequest().mockRejectedValueOnce({ response: { data: { error: 'Invalid credentials' } } });

        await expect(authApi.login({ email: 'test', password: 'badpw' }))
    ...
    it('should handle network error', async () => {
        ...
        await expect(authApi.login({ email: 'test', password: 'pw' }))
            .rejects.toThrow('Network error');
    });

    it('should get files', async () => {
        testLogger.info('Testing listFiles')
        const files: FileListItem[] = [{ filename: 'file.pdf', url: 'http://example.com/file.pdf' }];
    ...
    it('should upload file', async () => {
        ...
        const fileResponse = { filename: 'new.pdf', url: 'http://example.com/new.pdf' };
    ...
    it('should handle error on getFile', async () => {
        ...
        getMockedRequest().mockRejectedValueOnce({ response: { data: { message: 'Not found' } } });

        await expect(uploadsApi.getFile('badfile.pdf'))
