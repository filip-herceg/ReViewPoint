...
describe('Type-Safe Upload API Client', () => {
    ...
    describe('uploadFile', () => {
        ...
        it('should handle upload errors', async () => {
            ...
            server.use(
                http.post('http://localhost:8000/api/v1/uploads', () => {
                    return HttpResponse.text('File too large', { status: 400, statusText: 'Bad Request' });
                })
            );

            const file = new File(['test content'], 'test.txt', { type: 'text/plain' });

            await expect(uploadApiClient.uploadFile(file)).rejects.toThrow('Upload failed: 400 Bad Request');
        });
    });
    ...
    describe('getFileInfo', () => {
        it('should get file info using generated client', async () => {
            ...
            expect(generatedApiClient.GET).toHaveBeenCalledWith('/api/v1/uploads/{filename}', {
                params: {
    ...
    describe('deleteFile', () => {
        it('should delete file using generated client', async () => {
            ...
            expect(generatedApiClient.DELETE).toHaveBeenCalledWith('/api/v1/uploads/{filename}', {
                params: {
                    path: { filename: 'test.txt' },
                },
            });
    ...
    describe('downloadFile', () => {
        it('should download file by first getting info then fetching content', async () => {
            ...
            server.use(
                http.get('http://localhost:8000/api/v1/uploads/test.txt/download', () => {
