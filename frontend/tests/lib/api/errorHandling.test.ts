...
import { handleApiError, HandledError } from '@/lib/api/errorHandling';
...
describe('handleApiError (logger and edge cases)', () => {
    ...
    it('calls logger.error for network error', () => {
        ...
        expect(logger.error).toHaveBeenCalledWith('Network error', expect.any(Error));
    });
    ...
    it('calls logger.error for unknown error type', () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith('Object with message property:', 'Something went wrong');
    });
    ...
    it('handles non-Error thrown value (object)', () => {
        ...
        expect(logger.error).toHaveBeenCalledWith('Unknown error type', expect.any(Error));
    });
