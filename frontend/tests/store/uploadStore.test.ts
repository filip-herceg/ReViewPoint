...
import { createUpload } from '@/lib/api' // Adjust the import based on your project structure
...
vi.mock('@/lib/api', () => ({
    uploadsApi: {
        ...
        createFile: vi.fn().mockResolvedValue({ id: '1', name: 'test.txt', status: 'pending', progress: 0, createdAt: new Date().toISOString() }),
    },
...
describe('uploadStore error handling', () => {
    ...
    it('sets error on fetchUploads API error', async () => {
        ...
        uploadsApi.listFiles = vi.fn().mockRejectedValue(new Error('API fetch failed'));
        ...
        expect(useUploadStore.getState().error?.message).toMatch(/API fetch failed/);
    });

    it('sets error on createUpload API error', async () => {
        const { uploadsApi } = await import('@/lib/api');
        uploadsApi.uploadFile = vi.fn().mockRejectedValue(new Error('Invalid upload object'));

        await act(async () => {
            await useUploadStore.getState().createUpload({ name: '', status: 'pending', progress: 0 });
        });

        expect(useUploadStore.getState().error).toBeTruthy();
        expect(useUploadStore.getState().error?.message).toMatch(/Invalid upload object/);
    });
    ...
    it('sets error on patchUpload API error', async () => {
        ...
        const upload = { id: '1', name: 'test.txt', status: 'pending' as const, progress: 0, createdAt: new Date().toISOString() };
    ...
    it('sets error on removeUpload API error', async () => {
        const { uploadsApi } = await import('@/lib/api');
        const mockDeleteFile = vi.fn().mockRejectedValue(new Error('Delete failed'));
