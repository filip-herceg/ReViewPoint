...
describe('authStore error handling', () => {
    ...
    it('throws if login called with missing user', () => {
        expect(() => useAuthStore.getState().login(null as any, tokens)).toThrow(/user and tokens required/i);
    });

    it('throws if login called with missing token', () => {
        expect(() => useAuthStore.getState().login(user, null as any)).toThrow(/user and tokens required/i);
    });
