...
describe('logger', () => {
    ...
    it('logs error, warn, info, debug, trace at debug level', () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith('[ERROR]', 'err', '{"foo":"bar"}', errObj);
        expect(warnSpy).toHaveBeenCalledWith('[WARN]', 'warn', '123');
        expect(infoSpy).toHaveBeenNthCalledWith(1, expect.stringMatching(/\[TEST-LOG\] \[info\] \[.*\]/), 'Set LOG_LEVEL', 'debug');
        expect(infoSpy).toHaveBeenNthCalledWith(2, '[INFO]', 'info', 'true');
        expect(infoSpy).toHaveBeenNthCalledWith(3, expect.stringMatching(/\[TEST-LOG\] \[info\] \[.*\]/), 'Set LOG_LEVEL', 'trace');
